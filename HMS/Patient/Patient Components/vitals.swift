...
struct VitalsView: View {
    ...
    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 20) {
                // Steps
                HealthVitalView(title: "Steps", value: steps.map { "\(Int($0))" } ?? "NA", unit: "", color: Color.blue.opacity(0.7), imageName: "figure.walk")
                
                // Heart Rate
                HealthVitalView(title: "Heart Rate", value: heartRate.map { "\(Int($0))" } ?? "NA", unit: "bpm", color: Color.green.opacity(0.7), imageName: "heart")
                
                // SpO2
                HealthVitalView(title: "SpO2", value: spo2.map { "\(Int($0))" } ?? "NA", unit: "%", color: Color.orange.opacity(0.7), imageName: "waveform.path.ecg")
                
                // Blood Pressure
                if let systolic = systolic, let diastolic = diastolic {
                    HealthVitalView(title: "Blood Pressure", value: "\(Int(systolic))/\(Int(diastolic))", unit: "mmHg", color: Color.purple.opacity(0.7), imageName: "heart.text.square")
                } else {
                    HealthVitalView(title: "Blood Pressure", value: "NA", unit: "mmHg", color: Color.purple.opacity(0.7), imageName: "heart.text.square")
                }
            }
            .padding()
        }
